...
/**
 * Implementación Skiko de [drawBorderPattern] — compartida por Desktop (jvm),
 * Web (wasmJs) e iOS (native).
 *
 * Paridad completa con androidMain: muestrea el perímetro de la cara con
 * [org.jetbrains.skia.PathMeasure] (equivalente a android.graphics.PathMeasure) y
 * estampa las guardas (Fishtail, Diamonds, Chevron, Meander) a lo largo del borde.
 *
 * Los stamps y la matemática de muestreo viven en commonMain ([StampPattern.kt]);
 * lo único platform-specific es el PathMeasure de Skiko.
 */
actual fun DrawScope.drawBorderPattern(
    ...
    when (pattern) {
        ...
        BorderPattern.DoubleRing ->
            ...
            val measure = PathMeasure(flatFacePath.asSkiaPath(), forceClosed = true)
            ...
            withProjection {
                clipPath(flatFacePath) {
                    samples.forEach { (dist, spacingReal) ->
                        val pos = measure.getPosition(dist) ?: return@forEach
                        val tan = measure.getTangent(dist) ?: return@forEach
                        val flatPos = Offset(pos.x, pos.y)
                        val flatAngle = atan2(tan.y, tan.x) * 180f / PI.toFloat()
